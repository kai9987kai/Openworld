...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class DepthOfField : PostEffectsBase
    {
        ...
        {
            Vector3 pos = (worldDist - cachedCamera.nearClipPlane) * cachedCamera.transform.forward + cachedCamera.transform.position;
            return cachedCamera.WorldToViewportPoint(pos).z / (cachedCamera.farClipPlane - cachedCamera.nearClipPlane);
        }
        ...
        private void WriteCoc(RenderTexture fromTo, bool fgDilate)
        {
            ...
            if (nearBlur && fgDilate)
            {
                ...
                // Capture foreground circle-of-confusion (CoC)
            ...
            {
                // Capture full CoC in alpha channel
                fromTo.MarkRestoreExpected(); // only touching alpha channel, RT restore expected
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            // Prepare temporary render textures
            ...
            {
                // DX11 bokeh scatter optimized path
                if (highResolution)
                {
                    internalBlurWidth = Mathf.Max(internalBlurWidth, 0.1f);
                    ...
                    // Capture foreground CoC if needed
                    if (nearBlur)
                    ...
                    // IMPORTANT: Reset the append buffer counter before use
                    if (cbPoints != null) cbPoints.SetCounterValue(0);

                    // Collect bokeh candidates and darken the pixel
                    Graphics.SetRandomWriteTarget(1, cbPoints);
                    Graphics.Blit(source, rtLow, dx11bokehMaterial, 0);
                    Graphics.ClearRandomWriteTargets();

                    // Foreground CoC blur after collection
                    if (nearBlur)
                    {
                        ...
                        // Merge foreground CoC with background CoC
                        Graphics.Blit(rtSuperLow2, rtLow, dofHdrMaterial, 3);
                    }

                    // Lay out alpha on destination target for nicer outlines
                    Graphics.Blit(rtLow, dest2, dofHdrMaterial, 20);

                    // Box blur pass to merge with bokeh buffer
                    ...
                    Graphics.Blit(dest2, destination); // workaround for DX11 high resolution flip issue
                ...
                else
                {
                    ...
                    // Capture CoC & color at low resolution
                    WriteCoc(source, false);
                    ...
                    // Reset append buffer counter before use
                    if (cbPoints != null) cbPoints.SetCounterValue(0);

                    // Collect bokeh candidates
                    Graphics.SetRandomWriteTarget(1, cbPoints);
